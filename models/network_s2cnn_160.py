...
class Res4_S2FPN(nn.Module):
    def __init__(self, in_channels_list, out_channels):
        super(Res4_S2FPN, self).__init__()
        leaky = 0
        if out_channels <= 64:
            leaky = 0.1
        ...
        # self.SE4 = SELayer(out_channels * 4)
        ...
        self.FPN1 = nn.Sequential(
            ...
            # S2Net(in_channel=out_channels, f1=1, f2=out_channels, b_in=90, b_l1=20, b_l2=80),
        ...
        self.FPN3 = nn.Sequential(
            ...
            S2Net(in_channel=out_channels, f1=1, f2=out_channels, b_in=60, b_l1=10, b_l2=20),
        )

        self.FPN4 = nn.Sequential(
            conv_bn1X1(in_channels_list[3], out_channels, stride=1, leaky=leaky),
    ...
    def forward(self, input):
        ...
        # input[3] = self.SE4(input[3])
        # '''End'''
        output1 = self.FPN1(input[0])  # 输出为[24, 256, 160, 160]
        output2 = self.FPN2(input[1])  # 输出为[24, 256, 80, 80]
        output3 = self.FPN3(input[2])  # 输出为[24, 256, 40, 40]
        output4 = self.FPN4(input[3])  # 输出为[24, 256, 20, 20]
...
class SE_S2FPN(nn.Module):
    def __init__(self, in_channels_list, out_channels):
        super(SE_S2FPN, self).__init__()
        leaky = 0
        if out_channels <= 64:
            leaky = 0.1

        self.SE2 = SELayer(out_channels)
        self.SE3 = SELayer(out_channels)
        ...
        self.FPN2 = nn.Sequential(
            conv_bn1X1(in_channels_list[1], out_channels // 2, stride=1, leaky=leaky)
        )
        self.S2Net2 = S2Net(in_channel=in_channels_list[1], f1=1, f2=out_channels // 2, b_in=90, b_l1=20, b_l2=20)

        self.FPN3 = nn.Sequential(
            conv_bn1X1(in_channels_list[2], out_channels // 2, stride=1, leaky=leaky),
        )
        self.S2Net3 = S2Net(in_channel=in_channels_list[2], f1=1, f2=out_channels // 2, b_in=60, b_l1=10, b_l2=10)
    ...
    def forward(self, input):
        ...
        output2_b1 = self.FPN2(input[1])  # 输出为[8,32,40,40]
        output2_b2 = self.S2Net2(input[1])  # 输出为[8,32,40,40]

        output2 = torch.cat((output2_b1, output2_b2), 1)  # 输出为[8,64,40,40]
        output2 = self.SE2(output2)  # 输出为[8,64,40,40]

        output3_b1 = self.FPN3(input[2])  # 输出为[8,32,20,20]
        output3_b2 = self.S2Net3(input[2])  # 输出为[8,32,20,20]

        output3 = torch.cat((output3_b1, output3_b2), 1)  # 输出为[8,64,20,20]
        output3 = self.SE3(output3)  # 输出为[8,64,20,20]

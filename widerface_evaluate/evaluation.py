...
def get_gt_boxes(gt_dir):
    ...
    gt_mat = loadmat(os.path.join(gt_dir, 'wider_val_360.mat'))
...
def image_eval(pred, gt, iou_thresh):
    ...
    _pred[:, 2] = _pred[:, 2] + _pred[:, 0]  # x坐标
    _pred[:, 3] = _pred[:, 3] + _pred[:, 1]  # y坐标
    _gt[:, 2] = _gt[:, 2] + _gt[:, 0]
    _gt[:, 3] = _gt[:, 3] + _gt[:, 1]

    # 所有的pred与gt之间计算overlap
    ...
    for h in range(_pred.shape[0]):
        ...
        # np.where(condition)：输出满足条件 (即非0) 元素的坐标，这里的坐标以tuple的形式给出，通常原数组有多少维，输出的tuple中就包含几个数组，分别对应符合条件元素的各维坐标
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--pred', default="../../public/wider360/val/wider360_txt/")
    parser.add_argument('-g', '--gt', default='../../public/wider360/val/ground_truth/')
